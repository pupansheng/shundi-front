			...
			<view class="register">
				...
				<cmd-transition name="fade-up">
					<view v-if="status">
						...
						<view class="register-code"><cmd-input v-model="mobile.code" type="number" maxlength="6" placeholder="请输入短信验证码"></cmd-input></view>
						<button class="btn-register" :class="registerMobile ? 'btn-register-active' : ''" hover-class="btn-register-hover" @tap="fnRegister">注册</button>
					</view>
				</cmd-transition>
				<!-- #endif -->
				<!-- #ifndef H5 -->
				...
				<cmd-transition name="fade-up">
					<view v-if="!status">
						...
						<button class="btn-register" :class="registerAccount ? 'btn-register-active' : ''" hover-class="btn-register-hover" @tap="fnRegister">注册</button>
					</view>
				</cmd-transition>
				<!-- #endif -->
				<!-- #ifndef H5 -->
	...
	methods: {
		...
		fnGetPhoneCode() {
			if (this.phoneReg.test(this.mobile.phone)) {
				...
				let submit={};
				submit.url='/message/getYanzhengma/' + this.mobile.phone + '/register';
