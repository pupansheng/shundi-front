<template>
	<view class="message-list">
		<block v-if="messagesList.length==0">
			<view style="text-align: center;margin-top: 200rpx;">暂无聊天信息</view>
		</block>
					...
					<view class="uni-swipe-action__content " @click="toMessageDetail(i,it.type)">
						<view class="item" :class="it.stick  ? 'stick' : ''">
							...
							<block v-if="it.type == 2 || it.type == 3">
								<view class="item-left">
									<avator-group :type ="it.type" :avator = "it.name=='系统通知'?it.url:serverUrl+'/'+it.url"></avator-group>
								</view>
								<view class="item-middle">
									<text class="title">{{it.title}}</text>
									<text class="message">{{it.message}}</text>
...
<script>
	...
	export default {
		...
		computed:mapState(['linkList','userName','serverUrl']),
		...
		methods: {
			...
			toMessageDetail(i,type){
					if(type == 2){
						console.log('进入到聊天界面:'+this.linkList[i].title)
							uni.navigateTo({
							url:"../HM-chat/HM-chat?id="+i
							})
						}else if(type == 1){
				...
				uni.createSelectorQuery().in(this).select(`#${this.elId}`).boundingClientRect().exec((ret) => {
					...
					uni.getSystemInfo({
					    success: function (res) {
					     that.btnGroupWidth= res.windowWidth;
					    }
					});
				});
			},
